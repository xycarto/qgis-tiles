...
ovr_ext = ".ovr"
ovr_name = f"tiles/cog-outputs/{int(root_scale)}k.tif"

for scale in sorted_scales:
    vrt_list = []
    if scale != root_scale:
        for f in glob.glob(f'tiles/cog-outputs/**/{str(scale)}_gtiff_images.tif', recursive=True):
            vrt_list.append(f)
    ...
    # Process Root Scale    
        ...
        # Export VRT as Image
        gdal.Translate(
            f"tiles/cog-outputs/{str(scale)}k.tif",        
            f"tiles/cog-outputs/{str(scale)}k.vrt",
            format = "GTiff",
            resampleAlg = "bilinear",
            creationOptions = creation_options,
...
gdal.Translate(
    f"tiles/cog-outputs/{str(root_scale)}k_cog.tif",        
    f"tiles/cog-outputs/{str(root_scale)}k.tif",
    format = "COG",
    creationOptions = creation_options,
    callback = gdal.TermProgress_nocb 
)
